//基础url，后边连uuid
let base_url = "https://api.hypixel.net/player?key=这里填api key&uuid=";
...
function printText2(){
    ...
    for (let user of user_stat.keys()){
        ...
        let stats = JSON.parse(user_stat.get(user));
        
        //定义一个变量，用于显示查询到的状态
        let bwkills = "";
        if (stats.success){ //查询成功
         bwkills = stats.player.stats.Bedwars['kills_bedwars'];
         bwwins = stats.player.stats.Bedwars['wins_bedwars'];
         bwfk =  stats.player.stats.Bedwars['final_kills_bedwars'];
         bwfd = stats.player.stats.Bedwars['final_deaths_bedwars'];
         bwfkdr = bwfk/bwfd;
         bwfkdrfixed = bwfkdr.toFixed(3);
         bwlost = stats.player.stats.Bedwars['losses_bedwars']
         bwwl1 = bwwins/bwlost;
         bwwl = bwwl1.toFixed(2);
         rank = stats.player.newPackageRank;
         winstreak = stats.player.stats.Bedwars.winstreak;
            } 

        //使用模板字符串拼接字符串
        
        text += `
${rank}|${user}|Kills:${bwkills}|Wins:${bwwins}|FKDR:<font color="red">${bwfkdrfixed}</font>|W/L:${bwwl}|Winstreak:${winstreak}\n`;
    }
